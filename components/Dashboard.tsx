...
const Dashboard: React.FC<{ onNavigate: (target: { tab: NavItem, clientId?: string }) => void }> = ({ onNavigate }) => {
  ...
  const [whatsappConfig, setWhatsappConfig] = useState(() => {
    try {
      const saved = localStorage.getItem('grupo3a_whatsapp_config');
      return saved ? JSON.parse(saved) : { apiUrl: '', phone: '', token: '' };
    } catch {
      return { apiUrl: '', phone: '', token: '' };
    }
  });
  ...
  const fetchDashboardData = async () => {
    ...
    // Verificar se o caixa de hoje já está fechado
    try {
      const todayStr = new Date().toISOString().split('T')[0];
      const { data: fechamento } = await supabase
        .from('caixa_fechamentos')
        .select('*')
        .eq('data_fechamento', todayStr)
        .maybeSingle();
      setCaixaFechadoHoje(!!fechamento);
    } catch (e) {
      console.warn('Erro ao checar fechamento de caixa:', e);
    }
  ...
  const handleFecharCaixa = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('caixa_fechamentos').insert([{
      ...
      if (error) {
        if (error.code === '23505') {
          alert('Atenção: O caixa de hoje já foi fechado!');
      ...
      // Enviar mensagem para WhatsApp se configurado
      let whatsappEnviado = false;
      const { apiUrl, phone, token } = whatsappConfig;

      if (!apiUrl || !phone) {
        alert('Aviso: WhatsApp Business não está configurado. O fechamento do caixa foi registrado, mas o resumo financeiro não pôde ser enviado por WhatsApp.');
      } else {
        const todayDate = new Date();
        const formattedDate = todayDate.toLocaleDateString('pt-BR');
        
        // Calcular entradas e saídas do dia corrente
        const todayTransactions = transactions.filter(t => t.date === todayStr);
        const entradasDia = todayTransactions.filter(t => t.type === 'in').reduce((acc, t) => acc + Number(t.value || 0), 0);
        const saidasDia = todayTransactions.filter(t => t.type === 'out').reduce((acc, t) => acc + Number(t.value || 0), 0);
        
        const saldoTotalCaixa = financialData.consolidatedBalance;
        const saldoFinal = financialData.consolidatedBalance;

        const formatCurrency = (val: number) => {
          return val.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
        };

        const messageText = `FECHAMENTO DE CAIXA\n\nData: ${formattedDate}\n\nSaldo Total em Caixa:\n${formatCurrency(saldoTotalCaixa)}\n\nEntradas:\n${formatCurrency(entradasDia)}\n\nSaídas:\n${formatCurrency(saidasDia)}\n\nSaldo Final:\n${formatCurrency(saldoFinal)}\n\nFechamento realizado com sucesso.`;
        ...
        try {
          ...
          if (response.ok) {
            ...
            await supabase.from('caixa_fechamentos').update({ whatsapp_enviado: true }).eq('data_fechamento', todayStr);
          } else {
            console.error('Erro ao enviar mensagem pelo WhatsApp: HTTP', response.status);
            alert('Aviso: Falha ao enviar o resumo por WhatsApp (Resposta HTTP ' + response.status + '). O fechamento do caixa foi concluído.');
          }
        } catch (err: any) {
          console.error('Erro de rede ao enviar WhatsApp:', err);
          alert('Aviso: Não foi possível conectar à API do WhatsApp. O fechamento do caixa foi concluído.');
        }
      }

      // Inserir notificação push
      const formattedDateLocal = new Date().toLocaleDateString('pt-BR');
      const { error: notifError } = await supabase.from('notificacoes').insert([{
        titulo: 'Fechamento do Caixa',
        mensagem: `O caixa do dia ${formattedDateLocal} foi fechado com sucesso por ${userName}.${whatsappEnviado ? ' (Resumo WhatsApp enviado)' : ''}`,
        tipo: 'caixa',
        lida: false
      }]);
  ...
  const handleTestarNotificacao = async () => {
    try {
      ...
      alert('Erro ao enviar notificação de teste: ' + err.message);
    }
  };

  const availableAssetsStats = useMemo(() => {
    const isAvailable = (status: string) => {
      const s = (status || '').toLowerCase();
      return s === 'available' || s === 'disponivel';
    };

    const availVehicles = motorcycles.filter(m => isAvailable(m.status)).length;
    
    const availImoveis = properties.filter(p => {
      const isKitnet = p.description.toLowerCase().includes('kitnet') || p.code.toLowerCase().includes('kitnet') || (p.tipo && p.tipo.toLowerCase().includes('kitnet'));
      return !isKitnet && isAvailable(p.status);
    }).length;

    const availKitnets = properties.filter(p => {
      const isKitnet = p.description.toLowerCase().includes('kitnet') || p.code.toLowerCase().includes('kitnet') || (p.tipo && p.tipo.toLowerCase().includes('kitnet'));
      return isKitnet && isAvailable(p.status);
    }).length;

    const total = availVehicles + availImoveis + availKitnets;

    return { total, vehicles: availVehicles, imoveis: availImoveis, kitnets: availKitnets };
  }, [properties, motorcycles]);
  ...
  const totalADevolver = useMemo(() => {
    return banks
      ...
      .reduce((acc, b) => acc + Math.abs(Number(b.balance || 0)), 0);
  }, [banks]);
  ...
  const financialData = useMemo(() => {
    ...
    // Agora reflete exatamente o saldo manual inserido no BankManagement (excluindo investimentos e contas a devolver).
    const consolidatedBalance = banks
      .filter(b => b.tipo_conta !== 'Investimento' && !b.name.toUpperCase().includes('DEVOLVER'))
      .reduce((total, bank) => total + Number(bank.balance || 0), 0);
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      >
        <div className="flex flex-wrap gap-2 justify-end">
          ...
          <button
            ...
            className="p-2.5 bg-brand-surface border border-slate-200 dark:border-slate-800 rounded-xl text-slate-600 dark:text-slate-300 shadow-sm hover:bg-slate-50 transition-all flex items-center justify-center"
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-xs font-black flex items-center gap-2 transition-all ${
              caixaFechadoHoje
                ...
                : 'bg-rose-600 text-white shadow-lg shadow-rose-600/20 hover:scale-[1.02] active:scale-95'
            }`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Bens Disponíveis */}
        <div className="bg-white dark:bg-brand-surface border border-slate-100 dark:border-slate-800 p-6 rounded-3xl shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-success font-black">lock_open</span>
              <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-widest text-xs font-black">Bens Disponíveis</h4>
            </div>
            <span className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-2 py-0.5 rounded-full text-[10px] font-black">
              Disponíveis
            </span>
          </div>
          <div className="flex flex-col justify-center items-center py-4 space-y-2">
            <span className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">{availableAssetsStats.total}</span>
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider text-center line-clamp-2">
              {availableAssetsStats.vehicles} veículos • {availableAssetsStats.imoveis} imóveis • {availableAssetsStats.kitnets} kitnets
            </span>
          </div>
        </div>
        ...
        {(alerts.fines > 0 || alerts.overdueCharges > 0 || totalADevolver > 0) && (
          <div className="md:col-span-2 lg:col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {totalADevolver > 0 && (
              <div className="bg-rose-50 dark:bg-rose-900/20 border-l-4 border-red-600 p-4 rounded-r-xl flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  <span className="material-symbols-outlined text-red-600 font-black">assignment_return</span>
                  <div>
                    <h4 className="font-black text-red-700 dark:text-rose-450 text-sm uppercase tracking-wider">A DEVOLVER</h4>
                    <p className="text-base font-black text-red-600 dark:text-rose-300">
                      R$ {totalADevolver.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </p>
                  </div>
                </div>
              </div>
      ...
      {isWhatsappConfigOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-brand-surface rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 dark:border-slate-800 relative animate-in fade-in duration-200">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
                  URL da API de Envio (Endpoint POST)
                </label>
                <input
                  type="text"
                  ...
                  className="bg-slate-50 dark:bg-slate-900/50 border border-slate-150 dark:border-slate-800 w-full px-4 py-2 rounded-xl text-xs font-medium outline-none transition-all focus:bg-white dark:focus:bg-slate-900 focus:border-primary/45"
                />
              </div>

              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
                  Token de Autorização (Bearer Token / Opcional)
                </label>
                <input
                  type="text"
                  placeholder="Bearer token caso a API exija"
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
                  Telefone de Destino (Com DDD + DDI)
                </label>
                <input
                  type="text"
                  ...
                  className="bg-slate-50 dark:bg-slate-900/50 border border-slate-150 dark:border-slate-800 w-full px-4 py-2 rounded-xl text-xs font-medium outline-none transition-all focus:bg-white dark:focus:bg-slate-900 focus:border-primary/45"
                />
                <span className="text-[9px] text-slate-400 mt-1 block">O número de WhatsApp que receberá a mensagem de resumo.</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mt-6">
              <button
                onClick={() => {
                  localStorage.setItem('grupo3a_whatsapp_config', JSON.stringify(whatsappConfig));
                  setIsWhatsappConfigOpen(false);
                  alert('Configurações salvas com sucesso!');
                }}
                className="flex-1 py-2.5 bg-primary text-slate-900 rounded-xl text-xs font-black hover:bg-primary/95 active:scale-95 transition-all uppercase tracking-wider"
              >
                Salvar Configurações
              </button>
              <button
                onClick={() => setIsWhatsappConfigOpen(false)}
                className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl text-xs font-bold hover:bg-slate-200 transition-all uppercase tracking-wider"
              >
                Cancelar
              </button>
            </div>

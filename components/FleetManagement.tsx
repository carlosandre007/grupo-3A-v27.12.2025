...
const FleetManagement: React.FC = () => {
  ...
  const [maintenanceType, setMaintenanceType] = useState<'all' | 'credit' | 'debit'>('all');
  ...
  const fetchMotorcycles = async () => {
    ...
    if (!error && data) {
      const mapped = data.map((item: any) => ({
        ...
        valorPatrimonial: item.valor_patrimonial,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const dbData = {
      ...
      valor_patrimonial: formData.valorPatrimonial ? parseFloat(formData.valorPatrimonial) : 0,
      valor_atual: formData.valorAtual ? parseFloat(formData.valorAtual) : 0,
      data_aquisicao: formData.dataAquisicao || null
    };
  ...
  const handleEdit = (moto: Motorcycle) => {
    setFormData({
      ...
      valorPatrimonial: moto.valorPatrimonial?.toString() || '',
      valorAtual: moto.valorAtual?.toString() || '',
      dataAquisicao: moto.dataAquisicao || ''
    });
  ...
  const handleCloseModal = () => {
    ...
    setFormData({
      ...
      valorPatrimonial: '', valorAtual: '', dataAquisicao: ''
    });
  };
  ...
  const filteredMaintenance = maintenanceRecords.filter(rec => {
    const matchesSearch = rec.description.toLowerCase().includes(maintenanceSearch.toLowerCase());
    const matchesDate = maintenanceDate ? rec.date === maintenanceDate : true;
    const matchesType = maintenanceType === 'all' ? true : rec.type === maintenanceType;
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={isEditing ? "Editar Veículo" : "Novo Veículo"}>
        <div className="space-y-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div><label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Valor Patrimonial</label><input type="number" step="0.01" value={formData.valorPatrimonial} onChange={(e) => setFormData({...formData, valorPatrimonial: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl font-bold" placeholder="R$ 0,00" /></div>
              <div><label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Valor Atual</label><input type="number" step="0.01" value={formData.valorAtual} onChange={(e) => setFormData({...formData, valorAtual: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl font-bold" placeholder="R$ 0,00" /></div>
              <div><label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Data de Aquisição</label><input type="date" value={formData.dataAquisicao} onChange={(e) => setFormData({...formData, dataAquisicao: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl font-bold" /></div>
            </div>
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-6">
              <div className="flex flex-col gap-3 mb-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[10px] font-black text-slate-400 px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-full">{filteredMaintenance.length} Registro(s)</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <input value={maintenanceSearch} onChange={(e) => setMaintenanceSearch(e.target.value)} placeholder="Filtrar por texto..." className="px-3 py-2 bg-slate-50 dark:bg-slate-900 rounded-xl text-[10px] font-bold outline-none border border-transparent focus:border-primary/20" />
                  <input type="date" value={maintenanceDate} onChange={(e) => setMaintenanceDate(e.target.value)} className="px-3 py-2 bg-slate-50 dark:bg-slate-900 rounded-xl text-[10px] font-bold outline-none border border-transparent focus:border-primary/20" />
                  <select value={maintenanceType} onChange={(e) => setMaintenanceType(e.target.value as any)} className="px-3 py-2 bg-slate-50 dark:bg-slate-900 rounded-xl text-[10px] font-bold outline-none border border-transparent focus:border-primary/20">
                    <option value="all">Todos os tipos</option>
                    <option value="debit">Débitos / Despesas (-)</option>
                    <option value="credit">Créditos / Receitas (+)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto pr-1 custom-scrollbar">
                {filteredMaintenance.length === 0 ? (
                  <p className="text-center py-6 text-xs text-slate-400 font-bold uppercase">Nenhum registro encontrado</p>
                ) : (
